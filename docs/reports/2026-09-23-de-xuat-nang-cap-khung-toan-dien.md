...
| C6 | **Model ID cũ/không tồn tại trong lệnh người dùng gõ theo.** `audit-full.md:12` `/model claude-opus-4-8` và `claude-fable-5`; `completion.md:8` `claude-fable-5`; `CLAUDE.md:46` "Opus 4.8 / Fable 5"; `claude-opus-5` (legacy) ở bốn lệnh auto, adr, consult, incident, `models-and-automation.md:26`, `tiers.json:10`, `copy-framework.sh:207` trong khi ID hiện hành rẻ hơn là `claude-opus-5-5`. | grep | ✅ |
